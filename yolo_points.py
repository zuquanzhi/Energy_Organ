...
def build_mappings(config):
    ...
    for ann_id, definition in config["class_definitions"].items():
        ...
        if not isinstance(indices, list) or not all(isinstance(i, int) for i in indices):
            ...
            continue
        if len(indices) != 4:
            print(f"警告: 类别定义 (annotation_id: {ann_id}) 的索引数量不是 4，跳过。")
            continue
    ...
    for annotation in capture.get("annotations", []):
        ...
        if annotation_id in annotation_id_to_class_id:
            ...
            for instance in annotation.get("values", []):
                ...
                keypoints_map = {
                    kp['index']: kp for kp in instance.get("keypoints", [])
                    if 'index' in kp and 'location' in kp  # 只提取具有有效坐标的关键点
                }
                ...
                for index in required_indices:
                    if index in keypoints_map:
                        ...
                        print(f"警告: 实例 {instance_id}, 标注ID '{annotation_id}', 未找到索引 {index}")

                # 输出缺失的索引信息（如果需要调试）
                if missing_indices:
                    print(f"警告: 实例 {instance_id}, 标注ID '{annotation_id}', 缺失索引 {missing_indices}")

                # 如果找到的点数少于 4，跳过该实例
                if len(selected_points) >= 4:  # 至少找到 4 个有效点
                    ...
                    print(f"实例 {instance_id}, 标注ID '{annotation_id}', 找到 {len(selected_points)} 个有效点 ({found_indices})，缺失索引: {missing_indices}")
                else:
                    print(f"警告: 实例 {instance_id}, 标注ID '{annotation_id}', 找到的点数 ({len(selected_points)}) 不足 4，跳过。")

    return result

def convert_to_yolo_keypoints(data):
    """将提取的数据转换为 YOLO 格式：class_id + 8个归一化坐标 (x1 y1 x2 y2 x3 y3 x4 y4)"""
    ...
    for obj in data["objects"]:
        points = obj["points"]
        if len(points) != 4:
            print(f"警告: 实例 {obj.get('instance_id', 'N/A')} 的关键点数量不是 4 个，跳过。")
            continue
...
def process_files(config):
    ...
    output_dir = config.get("output", "output_yolo_keypoints")
    ...
    for json_path in sorted(json_files):
        ...
        if not extracted_data or not extracted_data["objects"]:
            ...
            continue
        # 转换为YOLO 格式
        yolo_keypoint_lines = convert_to_yolo_keypoints(extracted_data)
        if not yolo_keypoint_lines:
        ...
        try:
            ...
            processed_count += 1
            total_objects += len(yolo_keypoint_lines)
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="根据配置文件中指定的关键点索引将 Unity Perception JSON 转换为 YOLO 格式，每个对象包含 8 个归一化坐标点。")
